...
temp_data_bueno <-read.csv("TFBT-11.csv",sep = ",",dec = ".",header=T,fileEncoding="UTF-8-BOM")
...
edata_bueno <-read.csv("TFBB11.csv",sep = ";",dec = ",",header=T,skip =16)
...
colnames(edata_bueno)<- c("Fecha","Hora","Frecuencia","TVSec(V)","IPrim(A)","ISec(A)")
...
temp_data_malo <-read.csv("TFMT-12.csv",sep = ",",dec = ".",header=T,fileEncoding="UTF-8-BOM")
...
edata_malo <-read.csv("TFBM12.csv",sep = ";",dec = ",",header=T,skip =16)
...
#edata_malo <- edata_malo[,-(6:7)] ---- no es necesario en este caso

################# Eliminando la columna de tensión de neutro

# edata_malo <- edata_malo[,-(5)] ---- no es necesario en este caso

#Nombrando columnas variables eléctricas

colnames(edata_malo)<- c("Fecha","Hora","Frecuencia","TVSec(V)","IPrim(A)","ISec(A)")
...
# Agregando la columna de identificador de malo
...
ttesting <- DF_TINucleo[muestra,]

#tabla de aprendizaje del resto
taprendizaje <- DF_TINucleo[-muestra,]
...
# Pasando los valores a factores
real <- factor(ttesting[,9],levels = c("Bueno","Malo"))
...
# DF_cor <- DF_TINucleo[,-(1:2)] - No es necesario
...
ret <- write.csv(x=correlation, file="borrar.csv")
